...
#SBATCH --output=/home/rizk_lab/shared/kiran/kernel-specialization-3d-splatting/jobs/logs/phase3/ges/phase3b_ges_%A_%a.out
#SBATCH --error=/home/rizk_lab/shared/kiran/kernel-specialization-3d-splatting/jobs/logs/phase3/ges/phase3b_ges_%A_%a.err
...
mkdir -p "$PROJECT_ROOT/jobs/logs/phase3/ges"
...
LOCAL_SCRATCH=${SLURM_TMPDIR:-/tmp}/ges_phase3b_${SLURM_JOB_ID:-$$}_${SLURM_ARRAY_TASK_ID:-0}
...
echo "============================================================"
echo "GES SYNTHETIC PHASE III-B MULTI-SEED"
...
if [[ -d "$OUTPUT" && -n "$(find "$OUTPUT" -mindepth 1 -print -quit 2>/dev/null)" ]]; then
  echo "Refusing to overwrite non-empty requested output: $OUTPUT" >&2
  exit 1
fi
mkdir -p "$OUTPUT"
...
echo "GES Phase III-B task complete: $SCENE_ID at $(date)"
